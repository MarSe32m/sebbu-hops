...
public extension HOPSHierarchy {
    ...
    ///   - shiftType: Hierarchy shift type (nuHOPS). Default is .none.
    ...
    func nonLinearDyadicPropagator<Noise, WhiteNoise>(start: Double,
        ...
        var Heff = H(start)
        let solver = SRK2Solver<[Vector<Complex<Double>>], Complex<Double>>(initialState: [initialBraHierarchyState, initialKetHierarchyState, initialShiftVector], t0: start, dt: stepSize) { t, currentStates in
            ...
            for customOperator in braCustomOperators {
                Heff.add(customOperator(t, braState, ketState))
            }
            ...
            for customOperator in ketCustomOperators {
                Heff.add(customOperator(t, braState, ketState))
            }
