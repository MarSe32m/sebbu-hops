//
//  UnifiedNonLinearHOPSQSDState.swift
//  sebbu-hops
//
...
extension UnifiedHOPSHierarchy {
    @usableFromInline
    internal struct NonLinearNormalizedHOPSQSDStateFunc<Noise: ComplexNoiseProcess, WhiteNoise: ComplexWhiteNoiseProcess>: ~Copyable, ~Escapable, SDERHSFunction {
        ...
        public func diffusion(t: Double, y state: borrowing NonLinearNormalizedHOPSQSDState, channel: Int, into result: inout NonLinearNormalizedHOPSQSDState) {
            var resultPointer = result.totalStateVector.components
            var currentStatePointer = state.totalStateVector.components
            var index = 0
            while index < totalDimension {
        ...
        @inlinable
        public func zero() -> NonLinearNormalizedHOPSQSDState {
            return .init(dimension: totalDimension, shiftDimension: hierarchyPointer.pointee.G.count)
        }
    }
    
    @usableFromInline
    internal struct NonLinearNormalizedHOPSQSDState: ~Copyable, FixedStepSDESolverState {
        ...
        @inlinable
        public mutating func assign(_ other: borrowing UnifiedHOPSHierarchy.NonLinearNormalizedHOPSQSDState) {
            totalStateVector.copyComponents(from: other.totalStateVector)
            shiftVector.copyComponents(from: other.shiftVector)
        }
        
        @inlinable
        public mutating func add(_ a: borrowing UnifiedHOPSHierarchy.NonLinearNormalizedHOPSQSDState, multiplied: Double) {
            totalStateVector.add(a.totalStateVector, multiplied: multiplied)
            shiftVector.add(a.shiftVector, multiplied: multiplied)
        }
        
        @inlinable
        public mutating func add(_ a: borrowing UnifiedHOPSHierarchy.NonLinearNormalizedHOPSQSDState) {
            totalStateVector.add(a.totalStateVector)
            shiftVector.add(a.shiftVector)
        }
        
        @inlinable
        public mutating func assign(_ base: borrowing UnifiedHOPSHierarchy.NonLinearNormalizedHOPSQSDState, adding direction: borrowing UnifiedHOPSHierarchy.NonLinearNormalizedHOPSQSDState, multipliedBy c: Double) {
